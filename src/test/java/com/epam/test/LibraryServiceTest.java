...
import com.epam.web.service.impl.LibraryService;
import com.epam.web.service.impl.LibraryServiceImplService;


public class LibraryServiceTest {
	
	private static LibraryService SERVICE = new LibraryServiceImplService().getLibraryServiceImplPort();
	
	@Test
	public void getAllBooksSuccesTest(){
		LibraryWSResponse response = SERVICE.getAllBooks();
		List<Object> allBooks = response.getResult();
		assertNotNull(allBooks);
		assertEquals(allBooks.size(),10);
		
		String expectedStatus = "Get all books successfully";
